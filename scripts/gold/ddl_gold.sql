...
SELECT
	  ...
	  ,CASE WHEN ci.cst_gndr != 'n/a' THEN ci.cst_gndr --CRM is the master in case of differences between the 2 column genders
	  ELSE COALESCE(caz.gen , 'n/a')
	  END AS gender
	  ,caz.bdate AS birthdaydate
...
FROM [DataWarehouse].[silver].[crm_prd_info] pin
LEFT JOIN silver.erp_px_cat_g1v2 pcat
ON pin.cat_id = pcat.id
